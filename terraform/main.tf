...
resource "aws_security_group" "control" {
  name        = "ops3-control-sg-${var.onid}"
  description = "Ops3 control node: SSH admin access"
  vpc_id      = aws_vpc.ops3.id
...
# SSH only from control node SG
# TCP 25565 from public clients
resource "aws_security_group" "minecraft" {
  name        = "ops3-minecraft-sg-${var.onid}"
  description = "Ops3 Minecraft server: SSH from control, Minecraft public"
  vpc_id      = aws_vpc.ops3.id
...
resource "aws_instance" "control" {
  ...
  root_block_device {
    volume_size = var.control_root_volume_gib
    volume_type = "gp3"
  }

  tags = merge(local.common_tags, {
    Name = "ops3-control-${var.onid}"
